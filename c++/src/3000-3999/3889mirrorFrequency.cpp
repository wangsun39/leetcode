...
unordered_map<char, char> peer;

auto init = [] {
    for (char c='0';c<='9';c++) peer[c]='0'+(9-(c-'0'));
    for (char c='a';c<='z';c++) peer[c]='a'+(25-(c-'a'));
...
public:
    int mirrorFrequency(string s) {
        ...
        int ans=0;
        for (char c='0';c<='9';c++) ans+=abs(counter[c]-counter[peer[c]]);
        for (char c='a';c<='z';c++) ans+=abs(counter[c]-counter[peer[c]]);
        return ans/2;
    }
};
