...
public:
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& heights) {
        ...
        auto dfs = [&](this auto&& dfs, int i, int j) -> int {
            if (sa.find((i<<16)+j)!=sa.end()) return 3;
            vis[i][j]=1;
            int dir[4][2] = {{0, 1},{1,0},{0,-1},{-1,0}};
            int res=0;
            if (i==0||j==0) res=1;
            if (i==r-1||j==c-1) res|=2;
            for (int k=0;k<4;k++) {
                auto [dx, dy] = dir[k];
                int x=i+dx,y=j+dy;
                if (0<=x&&x<r&&0<=y&&y<c&&!vis[x][y]) {
                    if (heights[i][j]<heights[x][y]) continue;
                    if (x==0||y==0) res|=1;
                    if (x==r-1||y==c-1) res|=2;
                    res|=dfs(x,y);
                    if (res==3) break;
                }
            }
