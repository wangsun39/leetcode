...
class Solution:
    def extractMantra(self, matrix: List[str], mantra: str) -> int:
        ...
        @cache
        def dfs(i, x, y):  # 对于 mantra 从第 i 个字母开始的后缀，当前位置在[x, y],需要的最少移动次数
            # print(i, x, y)
            if i >= len(mantra):
                return 0
            res = inf
            if len(d[mantra[i]]) == 0:
                return res
            if i > 1 and mantra[i] == mantra[i - 1]:
                return dfs(i + 1, x, y) + 1
            for u, v in d[mantra[i]]:
                res = min(res, dfs(i + 1, u, v) + abs(x - u) + abs(y - v) + 1)
            return res
        ans = dfs(0, 0, 0)
        if ans == inf:
            return -1
...
print(so.extractMantra(matrix = ["abc","daf","geg"], mantra = "sad"))
