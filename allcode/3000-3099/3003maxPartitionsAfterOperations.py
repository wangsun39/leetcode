    ...
    def maxPartitionsAfterOperations(self, s: str, k: int) -> int:
        ...
        def dfs(i, mask, used):
            ...
            if mask2.bit_count() > k:
                res = dfs(i + 1, 1 << x, used) + 1
            else:
                res = dfs(i + 1, mask2, used)
            if not used:
                for y in range(26):
                    if y != x:
                        mask2 = mask | (1 << y)
                        if mask2.bit_count() > k:
                            res = max(res, dfs(i + 1, 1 << y, 1) + 1)
                        else:
                            res = max(res, dfs(i + 1, mask2, 1))
            # print(i, mask, used, res)
            return res
        return dfs(0, 0, 0)


so = Solution()
print(so.maxPartitionsAfterOperations(s = "accca", k = 2))
