...
restaurant_orders = pd.DataFrame(data, columns=[
    ...
    "order_amount",
    "payment_method",
    "order_rating"  # nullable integer for ratings that can be NULL
])


print(find_golden_hour_customers(restaurant_orders))

# -- Write your PostgreSQL query statement below

# select customer_id, count(1) total_orders,
# round(sum(case when EXTRACT(HOUR FROM order_timestamp)::int IN (11, 12, 13, 18, 19, 20) THEN 1 end) * 100.0 / count(*), 0) as peak_hour_percentage,
# round(avg(case when order_rating is not null then order_rating end), 2) as average_rating
# from restaurant_orders
# group by customer_id
# having count(1)>=3 and avg(case when order_rating is not null then order_rating end)>=4
#     and sum(case when EXTRACT(HOUR FROM order_timestamp)::int IN (11, 12, 13, 18, 19, 20) THEN 1 end) >= 0.6 * count(*)
#     and sum(case when order_rating is not null then 1 end) >= count(*) * 0.5
