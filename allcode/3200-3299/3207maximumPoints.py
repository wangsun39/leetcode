...
class Solution:
    def maximumPoints(self, enemyEnergies: List[int], currentEnergy: int) -> int:
        ...
        ans = 0
        while dq:
            if currentEnergy >= dq[0]:
                q, r = divmod(currentEnergy, dq[0])
                ans += q
                currentEnergy = r
            elif ans > 0:
...
so = Solution()
print(so.maximumPoints(enemyEnergies = [2], currentEnergy = 1000000000))
print(so.maximumPoints(enemyEnergies = [1,2,3], currentEnergy = 0))
print(so.maximumPoints(enemyEnergies = [3,2,2], currentEnergy = 2))
