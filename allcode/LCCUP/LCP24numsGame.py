...
class Solution:
    def numsGame(self, nums: List[int]) -> List[int]:
        ...
        for i, x in enumerate(nums[1:], 1):
            b = x - i  # 将b[i] 放入两个堆中，left是小的一半，right是大的一半
            # left是大顶堆，right是小顶堆
            if b < -left[0]:
                heappush(left, -b)
                sl += b
            ...
            if i & 1:  # 总数为偶数
                ...
                ans[i] = (sr - sl - left[0]) % MOD
        return ans



so = Solution()
print(so.numsGame([3,4,5,1,6,7]))
print(so.numsGame([1,2,3,4,5]))
print(so.numsGame([1,1,1,2,3,4]))
