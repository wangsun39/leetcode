...
class Solution:
    def electricCarPlan(self, paths: List[List[int]], cnt: int, start: int, end: int, charge: List[int]) -> int:
        ...
        def dijkstra(g: List[List[Tuple[int]]], start: int, n: int) -> List[int]:
            dist = [[inf] * (cnt + 1) for _ in range(n)]  # dist[i][j]  表示到达i，电量为j的最少耗时
            dist[start][0] = 0
            h = [(0, start, 0)]
            while h:
                d, x, c = heappop(h)
                if x == end: return dist  # 后面不会更少时间了
                if d > dist[x][c]:
                    continue
                for y, wt in g[x]:
                    for z in range(cnt - c + 1):
                        # z 充电数量
                        ze = z * charge[x]  # 充电时间
                        if wt > c + z: continue
                        new_d = dist[x][c] + wt + ze
                        if new_d < dist[y][c + z - wt]:
                            dist[y][c + z - wt] = new_d
                            heappush(h, (new_d, y, c + z - wt))
            return dist

        dist = dijkstra(g, start, len(g))
        return min(dist[end])


so = Solution()

print(so.electricCarPlan(paths = [[1,3,3],[3,2,1],[2,1,3],[0,1,4],[3,0,5]], cnt = 6, start = 1, end = 0, charge = [2,10,4,1]))
