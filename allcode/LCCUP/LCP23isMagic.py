...
class Solution:
    def isMagic(self, target: List[int]) -> bool:
        n = len(target)
        compare = list(range(2, n + 1, 2)) + list(range(1, n + 1, 2))
        k = 0
        while k < n and target[k] == compare[k]:
            k += 1
        if k == 0:
            return False
        ...
        def trans():
            ...
            while len(cur):
                a, b = [cur[i] for i in range(1, len(cur), 2)], [cur[i] for i in range(0, len(cur), 2)]
                c = a + b
                if len(c) > k:
                    res += c[:k]
                    cur = c[k:]
...
so = Solution()
print(so.isMagic(target = [5,4,3,2,1]))
print(so.isMagic(target = [2,4,3,1,5]))
