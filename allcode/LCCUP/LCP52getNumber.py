...
class Solution:
    def getNumber(self, root: Optional[TreeNode], ops: List[List[int]]) -> int:
        ...
        for op, x, y in ops[::-1]:
            p1 = sl.bisect_left(x)
            p2 = sl.bisect_right(y)
            if op == 0:
                for z in range(p1, p2):
                    sl.pop(p1)
            else:
                for z in range(p1, p2):
                    ans += 1
...
so = Solution()
print(so.getNumber(materials = [3,2,4,1,2], cookbooks = [[1,1,0,1,2],[2,1,4,0,0],[3,2,4,1,0]], attribute = [[3,2],[2,4],[7,6]], limit = 5))  # 7
