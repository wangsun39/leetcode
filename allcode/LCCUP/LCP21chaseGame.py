...
class Solution:
    def chaseGame(self, edges: List[List[int]], startA: int, startB: int) -> int:
        ...
        if startB in g[startA]: return 1

        dq = deque([x for x in range(n) if deg[x] == 1])
        not_in_loop = set(dq)
        while dq:
            x = dq.popleft()
            for y in g[x]:
                if y not in not_in_loop:
                    deg[y] -= 1
                    if deg[y] == 1:
                        not_in_loop.add(y)
                        dq.append(y)

        if startB not in not_in_loop:
            return -1
        ...
        def distance(x0):
            ...
            while dq:
                ...
                cur += 1
            return dist
        disA = distance(startA)
        if disA[connB] >= disB + 1 and n - len(not_in_loop) > 3:
            # 环小于4，B总会被抓住
            return -1

        # 剩下的情况，A总能抓住B
        disB = distance(startB)  # 重新定义disB，B到每个点的最短距离
...
so = Solution()
print(so.chaseGame(edges = [[1,2],[2,3],[3,4],[4,1],[2,5],[5,6]], startA = 3, startB = 5))
print(so.chaseGame(edges = [[1,2],[2,3],[3,1],[3,6],[2,4],[4,5],[5,8],[4,7]], startA = 8, startB = 7))
