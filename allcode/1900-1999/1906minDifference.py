...
class Solution:
    def minDifference(self, nums: List[int], queries: List[List[int]]) -> List[int]:
        ...
        for j, [l, r] in enumerate(queries):
            ...
            for i in range(mx):
                if l > 0:
                    diff[i] = counter[r][i] - counter[l - 1][i]
                else:
                    diff[i] = counter[r][i]
            mn = inf
            pre = -1
...
so = Solution()
print(so.minDifference(nums = [4,5,2,2,7,10], queries = [[2,3],[0,2],[0,5],[3,5]]))
print(so.minDifference(nums = [1,3,4,8], queries = [[0,1],[1,2],[2,3],[0,3]]))
