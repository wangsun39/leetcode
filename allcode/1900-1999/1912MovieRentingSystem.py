...
class MovieRentingSystem:
    ...
    def __init__(self, n: int, entries: List[List[int]]):
        self.dic_sm = {(shop, movie): price for shop, movie, price in entries}
        self.dic_m = defaultdict(SortedList)  # 未借出的电影
        for shop, movie, price in entries:
            self.dic_m[movie].add([price, shop])
        self.dic_ps = SortedList()   # 已借出的电影
    ...
    def search(self, movie: int) -> List[int]:
        ...
        # if len(sl) == 0:
        #     return []
        return [x for _, x in sl[:min(5, len(sl))]]


    def rent(self, shop: int, movie: int) -> None:
        price = self.dic_sm[(shop, movie)]
        self.dic_m[movie].remove([price, shop])
        self.dic_ps.add([price, shop, movie])


    def drop(self, shop: int, movie: int) -> None:
        price = self.dic_sm[(shop, movie)]
        self.dic_ps.remove([price, shop, movie])
        self.dic_m[movie].add([price, shop])


    def report(self) -> List[List[int]]:
        return [[shop, movie] for price, shop, movie in self.dic_ps[:min(5, len(self.dic_ps))]]
