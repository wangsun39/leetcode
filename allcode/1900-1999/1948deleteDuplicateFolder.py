...
class Solution:
    def deleteDuplicateFolder(self, paths: List[List[str]]) -> List[List[str]]:
        ...
        for i, path in enumerate(paths):
            if i < n - 1 and len(path) < len(paths[i + 1]): continue
            tr.insert(path)
...
so = Solution()
print(so.deleteDuplicateFolder(paths = [["a"],["c"],["d"],["a","b"],["c","b"],["d","a"]]))   # [["d"],["d","a"]]
