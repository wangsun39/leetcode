...
class Solution:
    def maxProfit(self, n: int, present: List[int], future: List[int], hierarchy: List[List[int]], budget: int) -> int:
        ...
        # sub[i][j][1] 以i为根的子树上，总预算为j时，选择i时的最大利润
        # sub[i][j][0] 以i为根的子树上，总预算为j时，不选择i时的最大利润
        sub = [[[0] * 2 for _ in range(budget + 1)] for _ in range(n)]


        # 计算子树x, 总预算b，且x的父节点是否选择为k时的最大利润
        def dfs2(x, b, k):
            res = sub[x][b][0]
...
so = Solution()
print(so.maxProfit(n = 2, present = [1,2], future = [4,3], hierarchy = [[1,2]], budget = 3))
