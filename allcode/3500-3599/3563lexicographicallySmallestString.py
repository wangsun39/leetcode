...
class Solution:
    def lexicographicallySmallestString(self, s: str) -> str:
        ...
        @cache
        def dfs(start, end):  # 返回s[start: end] 之间是否能被消除
            if start > end:  # 空串
                return True
            if (end - start + 1) & 1: return False  # 奇数个元素
            for i in range(start + 1, end, 2):
                if dfs(start, i) and dfs(i + 1, end):
                    return True
            if abs(s[start] - s[end]) in (1, 25):
                return dfs(start + 1, end - 1)
            return False

        @cache
        def dfs2(start):  # 返回子串 s[start:] 最小剩余串
            if start >= n - 1: return s[start:]
            if dfs(start, n - 1): return []
            res = [s[start]] + dfs2(start + 1)  # s[start] 剩余
...
so = Solution()
print(so.lexicographicallySmallestString("abc"))
