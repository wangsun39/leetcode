...
def seasonal_sales_analysis(products: pd.DataFrame, sales: pd.DataFrame) -> pd.DataFrame:
    ...
    ans = (
        sales
        .groupby(['season', 'category'], dropna=False)  # 显式保留 NaN
        .agg(
            total_quantity=('quantity', 'sum'),  # 新增：统计每组的行数
            total_revenue=('total_revenue', 'sum')
        )
    ).reset_index()
    ans = ans.sort_values(by=['season', 'total_quantity', 'total_revenue', 'category'], ascending=[True, False, False, True])

    return ans.drop_duplicates(subset=['season'], keep='first')
