...
class Solution:
    def canMakeEqual(self, nums: List[int], k: int) -> bool:
        ...
        nums1 = nums[:]
        return min(calc(nums, 1), calc(nums1, -1)) <= k


so = Solution()
print(so.canMakeEqual(nums = [-1,-1,-1,1,1,1], k = 5))
print(so.canMakeEqual(nums = [1,-1,1], k = 2))
print(so.canMakeEqual(nums = [-1,-1,1,-1,-1,-1,1,-1,1], k = 9))
print(so.canMakeEqual(nums = [1,-1,1,-1,1], k = 3))
