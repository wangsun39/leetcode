...
class Solution:
    def findDuplicate(self, paths: List[str]) -> List[List[str]]:
        ...
        for path in paths:
            ...
            for file in arr[1:]:
                p1 = file.find('(')
                name, cont = file[:p1], file[p1+1:-1]
                dic[cont].append(arr[0] + '/' + name)
        return [v for _, v in dic.items()if len(v) > 1]

so = Solution()
print(so.findDuplicate(paths = ["root/a 1.txt(abcd) 2.txt(efgh)","root/c 3.txt(abcd)","root/c/d 4.txt(efgh)","root 4.txt(efgh)"]))
