...
class Solution:
    def assignTasks(self, servers: List[int], tasks: List[int]) -> List[int]:
        ...
        for i, t in enumerate(tasks):
            while len(busy) and busy[0][0] <= i:
                x, w, y = heapq.heappop(busy)
                heapq.heappush(idle, [servers[y], y])  # idle = [[weight, idx]]
            if 0 == len(idle):
                x, w, y = heapq.heappop(busy)  # busy = [[endTime, idx]]
                heapq.heappush(busy, [x + t, w, y])
            else:
                x, y = heapq.heappop(idle)
                heapq.heappush(busy, [i + t, servers[y], y])
            ans.append(y)
...
print(so.assignTasks(servers = [338,890,301,532,284,930,426,616,919,267,571,140,716,859,980,469,628,490,195,664,925,652,503,301,917,563,82,947,910,451,366,190,253,516,503,721,889,964,506,914,986,718,520,328,341,765,922,139,911,578,86,435,824,321,942,215,147,985,619,865],
...
print(so.assignTasks(servers = [5,1,4,3,2], tasks = [2,1,2,4,5,2,1]))
