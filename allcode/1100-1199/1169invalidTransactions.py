...
class Solution:
    def invalidTransactions(self, transactions: List[str]) -> List[str]:
        ...
        for i in range(n):
            s1 = transactions[i]
            name1, t1, p1, c1 = s1.split(',')
            if int(p1) > 1000:
                ans.add(i)
            for j in range(i):
                s2 = transactions[j]
                name2, t2, p2, c2 = s2.split(',')
                if name1 == name2 and c1 != c2:
                    if abs(int(t1) - int(t2)) <= 60:
                        ans.add(i)
                        ans.add(j)
...
so = Solution()
print(so.invalidTransactions(transactions = ["alice,20,800,mtv","alice,50,100,beijing"]))
