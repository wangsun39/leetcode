...
Array.prototype.snail = function(rowsCount, colsCount) {
    ...
    const matrix = new Array(rowsCount).fill(0).map(() => new Array(colsCount).fill(0));

    for (let i = 0; i < n; i++) {
        let v=Math.floor(i/rowsCount);
        if ((v % 2)==0)
            matrix[i%rowsCount][v]=this[i];
        else
            matrix[rowsCount-1-i%rowsCount][v]=this[i];
    }
