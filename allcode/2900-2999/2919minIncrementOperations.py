...
class Solution:
    def minIncrementOperations(self, nums: List[int], k: int) -> int:
        n = len(nums)
        dp1 = max(0, k - nums[2])  # 当前位置i时，如果nums[i]要提升到k，那么前i个数的总操作最小值
        dp2 = max(0, k - nums[1])  # 当前位置i时，如果nums[i-1]要提升到k，那么前i个数的总操作最小值
        dp3 = max(0, k - nums[0])  # 当前位置i时，如果nums[i-2]要提升到k，那么前i个数的总操作最小值
