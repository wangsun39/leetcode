...
def find_customers(visits: pd.DataFrame, transactions: pd.DataFrame) -> pd.DataFrame:
    ...
    ans = (
        ...
        .agg(
            count_no_trans=('visit_id', 'size'),  # 新增：统计每组的行数
        )
    ).reset_index()
    return ans
...
# PostgreSQL
# select customer_id, count(1) count_no_trans from visits where visit_id not in (select visit_id from Transactions) group by customer_id;
