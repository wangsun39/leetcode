...
def analyze_organization_hierarchy(employees: pd.DataFrame) -> pd.DataFrame:
    ...
    def build(row):
        ...
        row_cp = row.copy()  # row 传入的是引用，所以要copy
        if pd.isna(row['manager_id']):
            g[-1].append(row_cp)
        else:
            g[row['manager_id']].append(row_cp)

    ans = employees.assign(level=0, team_size=0, budget=0)

    ans.apply(build, axis=1)  # 按行重新处理

    def dfs(row, lv):
        sz = 0
        buget = row['salary']
        for nxt_row in g[row['employee_id']]:
            r1, r2 = dfs(nxt_row, lv + 1)
            sz += r1 + 1
            buget += r2
        ans.loc[ans['employee_id'] == row['employee_id'], ['level', 'team_size', 'budget']] = lv, sz, buget
    ...
    ans = ans[['employee_id', 'employee_name', 'level', 'team_size', 'budget']]
    ans.sort_values(by=['level', 'budget', 'employee_name'], ascending=[True, False, True], inplace=True)
...
print(analyze_organization_hierarchy(employees))
