...
class Solution:
    def filterOccupiedIntervals(self, occupiedIntervals: List[List[int]], freeStart: int, freeEnd: int) -> List[List[int]]:
        ...
        ans = []
        for l, r in merged:
            if r + 1 < freeStart or freeEnd + 1 < l:
                ans.append([l, r])
                continue
            if l < freeStart:
...
so = Solution()
print(so.filterOccupiedIntervals(occupiedIntervals = [[76,80],[58,99],[48,57]], freeStart = 58, freeEnd = 86))
print(so.filterOccupiedIntervals(occupiedIntervals = [[2,6],[4,8],[10,10],[10,12],[14,16]], freeStart = 7, freeEnd = 11))
