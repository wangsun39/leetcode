...
# @functools.total_ordering  # 这个可以自动补齐le和gt/ge的函数
class RevStr:
    __slots__ = ('s',)
    def __init__(self, s):
        self.s = s
...
class SORTracker:
    ...
    def add(self, name: str, score: int) -> None:
        if len(self.prefix) == 0 and len(self.suffix) == 0:
            heappush(self.suffix, [-score, name])
            return
        if len(self.prefix) == 0 or self.prefix[0] > [score, RevStr(name)]:
            heappush(self.suffix, [-score, name])
            return
        sc, na = heappop(self.prefix)
        heappush(self.prefix, [score, RevStr(name)])
        heappush(self.suffix, [-sc, str(na)])

    def get(self) -> str:
        sc, na = heappop(self.suffix)
        heappush(self.prefix, [-sc, RevStr(na)])
        print(na, len(self.prefix), len(self.suffix))
        return na
