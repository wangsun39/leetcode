...
class Solution:
    def minimumRefill(self, plants: List[int], capacityA: int, capacityB: int) -> int:
        n = len(plants)
        leftA = capacityA - plants[0]
        leftB = capacityB - plants[-1]
        ans = 0
        for i in range(1, n // 2):
            if leftA >= plants[i]:
                ...
                ans += 1
            if leftB >= plants[n - 1 - i]:
                leftB -= plants[n - 1 - i]
            else:
                leftB = capacityB - plants[n - 1 - i]
                ans += 1
        if n & 1:
            if plants[n // 2] > max(leftA, leftB):
                ans += 1
        return ans

so = Solution()
print(so.minimumRefill(plants = [2,2,3,3], capacityA = 5, capacityB = 5))
