...
class Solution:
    def numSpecialEquivGroups(self, words: List[str]) -> int:
        s = set()
        for word in words:
            w1, w2 = sorted([word[i] for i in range(0, len(word), 2)]), sorted([word[i] for i in range(1, len(word), 2)])
...
so = Solution()
print(so.numSpecialEquivGroups(words = ["abcd","cdab","cbad","xyzz","zzxy","zzyx"]))
