...
class Solution:
    def countWinningSequences(self, s: str) -> int:
        ...
        def compete(a, b):
            d = {'F': 0, 'W': 1, 'E': 2}
            return mat[d[a]][d[b]]
        dp1 = [0] * (n * 2 + 3) # dp[i] 前i轮，且i轮出F时，B得分为i的数量
        dp2 = [0] * (n * 2 + 3) # dp[i] 前i轮，且i轮出W时，B得分为i的数量
        dp3 = [0] * (n * 2 + 3) # dp[i] 前i轮，且i轮出E时，B得分为i的数量
        dp4 = [0] * (n * 2 + 3) # dp[i] 前i轮，且i轮出F时，B得分为i的数量
        dp5 = [0] * (n * 2 + 3) # dp[i] 前i轮，且i轮出W时，B得分为i的数量
        dp6 = [0] * (n * 2 + 3) # dp[i] 前i轮，且i轮出E时，B得分为i的数量
        v = compete(s[0], 'F')
        dp1[v] = 1
        v = compete(s[0], 'W')
        dp2[v] = 1
        ...
        for i, x in enumerate(s[1:], 1):
            for j in range(-n, n + 1, 1):
                v = compete(x, 'F')
                dp4[j] = (dp2[j - v] + dp3[j - v]) % MOD
                v = compete(x, 'W')
                dp5[j] = (dp1[j - v] + dp3[j - v]) % MOD
                v = compete(x, 'E')
                dp6[j] = (dp1[j - v] + dp2[j - v]) % MOD
...
so = Solution()
print(so.countWinningSequences(s = "FFF"))
print(so.countWinningSequences(s = "FWEFW"))
