...
def last_passenger(queue: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(1, n + 1):
        w = queue[queue['turn'] == i].iloc[0]['weight']
        if total + w > 1000: break
        total += w
        ans = queue[queue['turn'] == i].iloc[0]['person_name']
...
# group by a.person_name having sum(b.weight)<=1000 order by sum(b.weight) desc limit 1;
