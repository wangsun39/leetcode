...
def restaurant_growth(customer: pd.DataFrame) -> pd.DataFrame:
    grouped = (
        customer
        .groupby(['visited_on'], dropna=False)  # 显式保留 NaN
        .agg(
            amount=('amount', 'sum')
        )
    ).reset_index()
    grouped = grouped.sort_values(by='visited_on')
    # print(grouped)
    cur = 0
    ans = []
    ...
    return pd.DataFrame(ans, columns=['visited_on', 'amount', 'average_amount'])
...
# select visited_on, amount, round(amount/7::numeric, 2) average_amount from
...
# WHERE visited_on - 6 >= (SELECT MIN(visited_on) FROM Customer);
