...
class Solution:
    def minCost(self, startPos: List[int], homePos: List[int], rowCosts: List[int], colCosts: List[int]) -> int:
        ...
        return sum(rowCosts[x] for x in range(sx + 1, hx + 1)) + sum(colCosts[x] for x in range(sy + 1, hy + 1))



so = Solution()
print(so.minCost(startPos = [1, 0], homePos = [2, 3], rowCosts = [5, 4, 3], colCosts = [8, 2, 6, 7]))
print(so.minCost(startPos = [0, 0], homePos = [0, 0], rowCosts = [5], colCosts = [26]))
