...
class Robot:
    ...
    def step(self, num: int) -> None:
        self.start = True
        n = (self.width + self.height) * 2 - 4
        self.cur = (self.cur + num) % n


    def getPos(self) -> List[int]:
        if self.cur < self.width:
            return [self.cur, 0]
        if self.cur <= self.width + self.height - 2:
            return [self.width - 1, self.cur - (self.width - 1)]
        if self.cur <= self.width * 2 + self.height - 3:
            return [self.width - 1 - (self.cur - (self.width + self.height - 2)), self.height - 1]
        return [0, self.height - 1 - (self.cur - (self.width * 2 + self.height - 3))]

    def getDir(self) -> str:
        if not self.start:
            return 'East'
        if 0 < self.cur <= self.width - 1:
            return 'East'
        if self.width - 1 < self.cur <= self.width + self.height - 2:
            return 'North'
        if self.width + self.height - 2 < self.cur <= self.width * 2 + self.height - 3:
            return 'West'
        return 'South'
